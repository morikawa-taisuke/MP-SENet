...
def inference(a):
    ...
    # print(f'check:{a.checkpoint_file}')
    ...
    with torch.no_grad():
        for i, index in enumerate(test_indexes):
            print(index)
            noisy_wav, _ = librosa.load(os.path.join(a.input_noisy_wavs_dir, index+'_16kHz.wav'), sr=h.sampling_rate)
...
def main():
    ...
    parser.add_argument('--output_dir', default='generated_files/subset_DEMAND')
